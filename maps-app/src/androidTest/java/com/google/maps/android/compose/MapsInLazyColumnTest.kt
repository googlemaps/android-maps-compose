...
class MapsInLazyColumnTests {
  ...
  private fun initMaps() {
    ...
    composeTestRule.setContent {
      ...
      LaunchedEffect(visibleItems) { visibleMapCount.value = visibleItems }
  ...
  @Before
  fun setUp() {
    cameraPositionStates =
      mapItems.associate { item ->
        item.id to
          CameraPositionState(position = CameraPosition.fromLatLngZoom(item.location, item.zoom))
      }
  }
