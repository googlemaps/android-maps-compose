    ...
    GoogleMap(cameraPositionState = cameraPositionState) {
      when (config.markerType) {
        MarkerType.CUSTOM_CONTENT_MARKER -> {
          ...
          MarkerComposable(state = markerState, anchor = Offset(0.5f, 1.0f)) {
        ...
        MarkerType.STANDARD_MARKER_WITH_SNIPPET -> {
          ...
          Marker(
            ...
            title = config.title,
            snippet =
              config.standardMarkerSnippet
                ?: "Standard Marker Snippet" // Snippet for the info window
          )
        }
      }
    }
