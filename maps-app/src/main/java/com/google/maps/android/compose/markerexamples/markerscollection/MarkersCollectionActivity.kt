...
/**
 * Demonstrates how to sync a data model with a changing collection of location markers using keys.
 *
 ...
 * `UpdatingNoDragMarkerWithDataModelActivity` example, which encapsulates [MarkerState] to provide
...
private fun Screen(dataModel: DataModel, modifier: Modifier = Modifier) =
  GoogleMapWithLocations(
    ...
    onAddLocation = { locationData -> dataModel.locationDataMap += LocationKey() to locationData },
    onDeleteLocation = { key -> dataModel.locationDataMap -= key }
  )

/**
 * A GoogleMap with locations represented by markers
 *
 * @param keyedLocationData model data for location markers with unique keys. Uses a [Collection]
 *   type to keep it independent of our data model.
...
private fun GoogleMapWithLocations(
  ...
  GoogleMap(
    ...
    Locations(keyedLocationData = keyedLocationData, onLocationClick = onDeleteLocation)
  }
}
