...
/**
 * A sealed class representing a group of related demo activities. Using a sealed class here allows
 * us to define a closed set of categories, which is ideal for a static list of demos. This ensures
 * that we can handle all possible categories in a type-safe way.
 *
 * @param title The title of the activity group.
 * @param activities The list of activities belonging to this group.
 */
sealed class ActivityGroup(@StringRes val title: Int, val activities: List<Activity>) {
  object MapTypes :
    ActivityGroup(
...
/**
 * A data class representing a single demo activity. This class serves as a model for each item in
 * the demo list.
 *
...
/**
 * The single source of truth for all the demo activity groups. This list is used to populate the
 * main screen.
 */
val allActivityGroups =
  listOf(
...
/**
 * A composable function that displays a collapsible list of demo activity groups. This is the main
 * UI component for the main screen.
 *
 * The list is built using a `LazyColumn` for performance, ensuring that only the visible items are
 * rendered. Each group is represented by a clickable `Card` that expands or collapses to reveal the
 * activities within it. This approach keeps the UI clean and organized, especially with a large
 * number of demos.
 *
 ...
 */
@Composable
fun DemoList(onActivityClick: (KClass<out ComponentActivity>) -> Unit) {
  ...
  LazyColumn {
    items(allActivityGroups) { group ->
      ...
      Column {
        ...
        GroupHeaderItem(group, isExpanded) { expandedGroup = (if (isExpanded) null else group) }

        // Animate the visibility of the activities within the group.
        AnimatedVisibility(visible = isExpanded) {
          Column(modifier = Modifier.padding(start = 16.dp, end = 16.dp)) {
            // Create a card for each activity in the group.
            group.activities.forEach { activity -> DemoActivityItem(onActivityClick, activity) }
          }
        }
      }
    }
  ...
  Card(
    modifier =
      Modifier.fillMaxWidth().padding(vertical = 4.dp).clickable {
        onActivityClick(activity.kClass)
      }
  ) {
...
private fun GroupHeaderItem(
  ...
  Card(
    ...
    modifier = Modifier.fillMaxWidth().padding(8.dp).clickable { onGroupClicked() }
  ) {
