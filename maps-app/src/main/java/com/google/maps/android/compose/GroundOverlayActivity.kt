...
class GroundOverlayActivity : ComponentActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    enableEdgeToEdge()
    setContent { MapsComposeSampleTheme { GroundOverlayScreen() } }
  }
}
...
fun GroundOverlayScreen() {
  ...
  Box(modifier = Modifier.fillMaxSize().systemBarsPadding()) {
    GoogleMap(modifier = Modifier.fillMaxSize(), cameraPositionState = cameraPositionState) {
      ...
      GroundOverlay(
        position =
          GroundOverlayPosition.create(LatLngBounds(LatLng(1.35, 103.86), LatLng(1.37, 103.88))),
      ...
      GroundOverlay(
        position =
          GroundOverlayPosition.create(LatLngBounds(LatLng(1.36, 103.89), LatLng(1.38, 103.91))),
        image = imageDescriptor,
        transparency = 0.5f,
        zIndex = 1f
      )
...
fun GroundOverlayControls(
  ...
  Surface(
    ...
    Column(modifier = Modifier.padding(16.dp)) {
      ...
      Text(
        text = "Transparency: ${String.format(Locale.current.platformLocale, "%.2f", transparency)}"
      )
      Slider(value = transparency, onValueChange = onTransparencyChange, valueRange = 0f..1f)
      Text(text = "Bearing: ${bearing.toInt()}°")
      Slider(value = bearing, onValueChange = onBearingChange, valueRange = 0f..360f)
    }
  }
}
