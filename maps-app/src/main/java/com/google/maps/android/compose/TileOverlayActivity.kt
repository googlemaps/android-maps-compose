...
/**
 * This composable demonstrates how to use a [TileOverlay] with a [TileProvider] that updates its
...
private fun UpdatedTileOverlay() {
  ...
  val tileProvider =
    remember(tileProviderIndex) {
      TileProvider { _, _, _ -> Tile(size, size, renderTiles(renderedIndex, size)) }
    }
...
/**
 * Helper function to dynamically generate a tile image. The [TileProvider] interface requires that
 * a [ByteArray] is returned for each tile. This function creates a [Bitmap], draws the current
 * [index] on it, and then compresses it into a [ByteArray] to be returned by the provider.
 */
