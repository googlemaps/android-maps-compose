 ...
 *
 * This only stores location position, for illustration, but could hold other data related to the
 * location.
 */
...
/**
 * Encapsulates mapping from data model to MarkerStates. Part of view model. MarkerStates are
 * relegated to an implementation detail. Use new [DraggableMarkersModel] instance if data model is
 * updated externally: MarkerStates are source of truth after initialization from data model.
 */
@Stable
private class DraggableMarkersModel(dataModel: Map<LocationKey, LocationData>) {
  ...
  private val markerDataMap: SnapshotStateMap<LocationKey, MarkerState> =
    dataModel.entries
      .map { (locationKey, locationData) -> locationKey to MarkerState(locationData.position) }
      .toMutableStateMap()
  ...
  @Composable
  fun Markers() =
    markerDataMap.forEach { (locationKey, markerState) ->
      key(locationKey) { LocationMarker(markerState, onClick = { deleteLocation(locationKey) }) }
    }

  /**
   * List of functions providing current positions of Markers.
   *
   * Calling from composition will trigger recomposition when Markers and their positions change.
   */
...
/**
 * Demonstrates how to sync a data model with a changing collection of draggable markers using keys,
 * while keeping a Polygon of the marker positions in sync with the current marker position.
 *
...
@Composable
private inline fun LocationMarker(markerState: MarkerState, crossinline onClick: () -> Unit) =
  Marker(
    state = markerState,
    draggable = true,
    onClick = {
