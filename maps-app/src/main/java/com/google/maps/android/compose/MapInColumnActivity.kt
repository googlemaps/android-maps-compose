...
class MapInColumnActivity : ComponentActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    setContent {
      ...
      // Use a LaunchedEffect keyed on the camera moving state to enable column scrolling when the
      // camera stops moving
...
fun MapInColumn(
  ...
  Surface(modifier = modifier, color = MaterialTheme.colors.background) {
    ...
    Column(
      Modifier.fillMaxSize().verticalScroll(rememberScrollState(), columnScrollingEnabled),
      horizontalAlignment = Alignment.Start
    ) {
      Spacer(modifier = Modifier.padding(10.dp))
      for (i in 1..20) {
        Text(text = "Item $i", modifier = Modifier.padding(start = 10.dp).testTag("Item $i"))
      }
      Spacer(modifier = Modifier.padding(10.dp))
                ...
                onTouchEvent = {
                  when (it.action) {
                    MotionEvent.ACTION_DOWN -> {
                      ...
                      Log.d(TAG, "MotionEvent ${it.action} - this never triggers.")
                      true
                    }
                  }
                }
