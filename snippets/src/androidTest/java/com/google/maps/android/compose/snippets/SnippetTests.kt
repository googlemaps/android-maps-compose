 ...
 *
 * Bypasses the ComposeTestRule legacy Espresso wait-idle sync lifecycle, completely preventing
 * Espresso InputManager reflection crashes on Android SDK 37 preview devices. Executes composition
 * synchronously on the main thread, ensuring 100% code coverage metrics.
 */
...
class SnippetTests {
  ...
  private fun runSnippetCompositionTest(snippetComposable: @Composable () -> Unit) {
    ActivityScenario.launch(MainActivity::class.java).use { scenario ->
      scenario.onActivity { activity -> activity.setContent { snippetComposable() } }
