 ...
 * [MapView]. This is a trick to enable [ComposeView] to start its composition, as it requires being
 * attached to a window. [onAddedToWindow] is called in place, and then [view] is removed from the
...
internal fun MapView.renderComposeViewOnce(
  ...
  startRenderingComposeView(view, parentContext).use { onAddedToWindow?.invoke(view) }
}

/**
 * Prepares [view] for a rendering by attaching it as a descendant of this [MapView]. This is a
 * trick to enable [ComposeView] to start its composition, as it requires being attached to a
 * window. A [ComposeUiViewRenderer.RenderHandle] is returned, which must be disposed after this
 * view no longer needs to render. Disposing removes [view] from the [MapView].
 */
...
public fun rememberComposeUiViewRenderer(): ComposeUiViewRenderer {
  ...
  return remember(compositionContext) {
    object : ComposeUiViewRenderer {
      ...
      override fun renderViewOnce(view: AbstractComposeView, onAddedToWindow: (() -> Unit)?) {
...
public interface ComposeUiViewRenderer {
   ...
   */
  public fun renderViewOnce(view: AbstractComposeView, onAddedToWindow: (() -> Unit)?)

  public fun startRenderingView(view: AbstractComposeView): RenderHandle
...
/**
 * A ViewGroup that prevents its children from being laid out or drawn. Used for adding ComposeViews
 * as descendants of a MapView without actually affecting the view hierarchy from the user's
 * perspective.
 */
