...
public class MarkerState private constructor(position: LatLng) {
   ...
   *
   * This property is backed by Compose state. It can be updated by the API user and by the API
   * itself: it has two potentially competing sources of truth.
   *
   * The API will not update the property unless [isDragging] `== true`, which will happen if and
   * only if a Marker is draggable and the end user is currently dragging it.
   */
  public var position: LatLng by mutableStateOf(position)

  /**
   * Reflects whether the end user is currently dragging the marker. Dragging can happen only if a
   * Marker is draggable.
   *
   ...
   *
   * Not backed by Compose state to accommodate [com.google.android.gms.maps.GoogleMap] special
   * semantics: only a single info window can be visible for the entire GoogleMap.
   *
   ...
   *
   * Not backed by observable Compose state to accommodate [com.google.android.gms.maps.GoogleMap]
   * special semantics: only a single info window can be visible for the entire GoogleMap.
   *
   * Only use from Compose Effect APIs, never directly from composition, to avoid unexpected
   * behavior from cancelled compositions.
   */
  ...
  public companion object {
     ...
     */
    @StateFactoryMarker
    public operator fun invoke(position: LatLng = LatLng(0.0, 0.0)): MarkerState =
     ...
     * This cannot be used to preserve marker info window visibility across configuration changes.
     */
    public val Saver: Saver<MarkerState, LatLng> =
      Saver(save = { it.position }, restore = { MarkerState(it) })
  }
}

/**
 * Uses [rememberSaveable] to retain [MarkerState.position] across configuration changes, for simple
 * use cases.
 *
 ...
 *
 * This function does not automatically update the MarkerState when the input parameters change. If
 * you need this implementation, use 'rememberUpdatedMarkerState'.
 */
@Composable
@Deprecated(
  message =
    "Use 'rememberUpdatedMarkerState' instead - It may be confusing to think " +
...
): MarkerState = rememberSaveable(key = key, saver = MarkerState.Saver) { MarkerState(position) }

/**
 * This function updates the state value according to the update of the input parameter, like
 * 'rememberUpdatedState'.
 *
 * This cannot be used to preserve state across configuration changes.
 */
@Composable
public fun rememberUpdatedMarkerState(position: LatLng = LatLng(0.0, 0.0)): MarkerState =
  remember { MarkerState(position = position) }.also { it.position = position }

/**
 * A composable for a marker on the map.
 *
 * @param state the [MarkerState] to be used to control or observe the marker state such as its
 *   position and info window
...
/**
 * A composable for a marker on the map wherein its entire info window can be customized. If this
 * customization is not required, use [com.google.maps.android.compose.Marker].
 *
 * @param state the [MarkerState] to be used to control or observe the marker state such as its
 *   position and info window
 * @param alpha the alpha (opacity) of the marker
 ...
 * @param onInfoWindowClose a lambda invoked when the marker's info window is closed
 * @param onInfoWindowLongClick a lambda invoked when the marker's info window is long clicked
 * @param content optional composable lambda expression for customizing the info window's content
 */
@Composable
@GoogleMapComposable
 ...
 * customized. If this customization is not required, use [com.google.maps.android.compose.Marker].
 *
 * @param keys unique keys representing the state of this Marker. Any changes to one of the key will
 *   trigger a rendering of the content composable and thus the rendering of an updated marker.
 * @param state the [MarkerState] to be used to control or observe the marker state such as its
 ...
 * @param onInfoWindowClose a lambda invoked when the marker's info window is closed
 * @param onInfoWindowLongClick a lambda invoked when the marker's info window is long clicked
 * @param infoContent optional composable lambda expression for customizing the info window's
 *   content
 * @param content composable lambda expression used to customize the marker's content
 */
@Composable
...
/**
 * A composable for a marker on the map wherein its info window contents can be customized. If this
 * customization is not required, use [com.google.maps.android.compose.Marker].
 *
 * @param state the [MarkerState] to be used to control or observe the marker state such as its
 *   position and info window
 ...
 * @param onInfoWindowClose a lambda invoked when the marker's info window is closed
 * @param onInfoWindowLongClick a lambda invoked when the marker's info window is long clicked
 * @param infoWindow optional composable lambda expression for customizing the entire info window.
 *   If this value is non-null, the value in infoContent] will be ignored.
 * @param infoContent optional composable lambda expression for customizing the info window's
 *   content. If this value is non-null, [infoWindow] must be null.
 */
@Composable
@GoogleMapComposable
  ...
  ComposeNode<MarkerNode, MapApplier>(
    factory = {
      val advancedMarkerOptions =
        AdvancedMarkerOptions().position(state.position).collisionBehavior(collisionBehavior)
      ...
      val marker = mapApplier?.map?.addMarker(advancedMarkerOptions) ?: error("Error adding marker")
      marker.tag = tag
      MarkerNode(
        compositionContext = compositionContext,
        marker = marker,
    ...
    update = {
      ...
      update(pinConfig) {
        if (icon == null && iconView == null) {
          this.marker.setIcon(pinConfig?.let { it1 -> BitmapDescriptorFactory.fromPinConfig(it1) })
        }
      }
      update(icon) {
        if (iconView == null) {
