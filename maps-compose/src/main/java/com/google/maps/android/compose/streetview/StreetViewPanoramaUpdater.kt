...
internal class StreetViewPanoramaPropertiesNode(
  ...
  override fun onAttached() {
    super.onAttached()
    panorama.setOnStreetViewPanoramaClickListener { eventListeners.onClick(it) }
    panorama.setOnStreetViewPanoramaLongClickListener { eventListeners.onLongClick(it) }
    panorama.setOnStreetViewPanoramaCameraChangeListener {
      cameraPositionState.rawPanoramaCamera = it
    }
    panorama.setOnStreetViewPanoramaChangeListener { cameraPositionState.rawLocation = it }
  }
...
internal inline fun StreetViewUpdater(
  ...
  val streetViewPanorama = (currentComposer.applier as StreetViewPanoramaApplier).streetViewPanorama
  ComposeNode<StreetViewPanoramaPropertiesNode, StreetViewPanoramaApplier>(
    ...
    set(isPanningGesturesEnabled) { panorama.isPanningGesturesEnabled = isPanningGesturesEnabled }
    set(isStreetNamesEnabled) { panorama.isStreetNamesEnabled = isStreetNamesEnabled }
    set(isUserNavigationEnabled) { panorama.isUserNavigationEnabled = isUserNavigationEnabled }
