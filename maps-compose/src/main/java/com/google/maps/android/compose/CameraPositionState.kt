 ...
 *
 * The camera position state is saved across configuration changes and process death, ensuring the
 * map retains its last position.
 *
 * @param init A lambda that is called when the [CameraPositionState] is first created to configure
 *   its initial state, such as its position or zoom level.
 */
@Composable
public inline fun rememberCameraPositionState(
  crossinline init: CameraPositionState.() -> Unit = {}
): CameraPositionState =
  rememberSaveable(saver = CameraPositionState.Saver) { CameraPositionState().apply(init) }

/**
 * Create and [rememberSaveable] a [CameraPositionState] using [CameraPositionState.Saver]. [init]
 * will be called when the [CameraPositionState] is first created to configure its initial state.
 * Remember that the camera state can be applied when the map has been loaded.
 */
@Deprecated(
  message =
...
public inline fun rememberCameraPositionState(
  ...
  rememberSaveable(key = key, saver = CameraPositionState.Saver) {
    CameraPositionState().apply(init)
  }

/**
 * A state object that can be hoisted to control and observe the map's camera state. A
 * [CameraPositionState] may only be used by a single [GoogleMap] composable at a time as it
 * reflects instance state for a single view of a map.
 *
 * @param position the initial camera position
 */
public class CameraPositionState
private constructor(position: CameraPosition = CameraPosition(LatLng(0.0, 0.0), 0f, 0f, 0f)) {
  internal var isLiteMode: Boolean = false

  /**
   * Whether the camera is currently moving or not. This includes any kind of movement: panning,
   * zooming, or rotation.
   */
   ...
   */
  public var cameraMoveStartedReason: CameraMoveStartedReason by
    mutableStateOf(CameraMoveStartedReason.NO_MOVEMENT_YET)
    internal set

  /**
   * Returns the current [Projection] to be used for converting between screen coordinates and
   * lat/lng.
   */
  ...
  /**
   * Local source of truth for the current camera position. While [map] is non-null this reflects
   * the current position of [map] as it changes. While [map] is null it reflects the last known map
   * position, or the last value set by explicitly setting [position].
   */
  ...
  /**
   * Used with [onMapChangedLocked] to execute one-time actions when a map becomes available or is
   * made unavailable. Cancellation is provided in order to resume suspended coroutines that are
   * awaiting the execution of one of these callbacks that will never come.
   */
   ...
   *
   * [animate] will throw [CancellationException] if the animation does not fully complete. This can
   * happen if:
   ...
   *
   * If this [CameraPositionState] is not currently bound to a [GoogleMap] this call will suspend
   * until a map is bound and animation will begin.
   *
    ...
    try {
      suspendCancellableCoroutine<Unit> { continuation ->
        synchronized(lock) {
          ...
          if (map == null) {
            // Do it later
            val animateOnMapAvailable =
              object : OnMapChangedCallback {
                override fun onMapChangedLocked(newMap: GoogleMap?) {
                  if (newMap == null) {
                    // Cancel the animate caller and crash the map setter
                    @Suppress("ThrowableNotThrown")
                    continuation.resumeWithException(
                      CancellationException("internal error; no GoogleMap available")
                    )
                    error("internal error; no GoogleMap available to animate position")
                  }
  ...
  private fun performAnimateCameraLocked(
    ...
    doOnMapChangedLocked {
      check(it == null) { "New GoogleMap unexpectedly set while an animation was still running" }
      map.stopAnimation()
    }
  }
   ...
   * or if the map is currently unbound, [update] will be applied when a map is next bound. Other
   * calls to [move], [animate], or setting [position] will override an earlier pending call to
   * [move].
   *
  ...
  public companion object {
    ...
    public val Saver: Saver<CameraPositionState, CameraPosition> =
      Saver(save = { it.position }, restore = { CameraPositionState(it) })
  }
}
